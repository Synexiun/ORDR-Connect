  ...
  return {
    getDashboardSummary: vi.fn().mockResolvedValue({
      ...
      data: {
        ...
        channelMetrics: [
          { channel: 'email', sent: 100, delivered: 95, failed: 5, deliveryRate: 0.95, avgCost: 0.02 },
        ],
        agentMetrics: [
          { agentRole: 'collections', sessions: 20, resolutions: 15, resolutionRate: 0.75, avgConfidence: 0.85, avgSteps: 3, avgCostCents: 12, avgDurationMs: 5000 },
        ],
        complianceMetrics: [
...
describe('Analytics Routes', () => {
  ...
  describe('GET /api/v1/analytics/trends/:metric', () => {
    ...
    it('returns agent performance trend', async () => {
      ...
      const res = await app.request(`/api/v1/analytics/trends/agent_performance?from=${from}&to=${to}`);
    ...
    it('returns 400 for invalid metric name', async () => {
      ...
      const res = await app.request(`/api/v1/analytics/trends/invalid_metric?from=${from}&to=${to}`);
  ...
  describe('GET /api/v1/analytics/real-time', () => {
    it('returns real-time counters', async () => {
      ...
      expect(res.status).toBe(200);
      const body = await res.json() as { success: boolean; data: Record<string, number>; timestamp: string };
      expect(body.success).toBe(true);
    ...
    it('passes tenantId from JWT to counters', async () => {
      ...
      expect(mockCounters.getMultiple).toHaveBeenCalledWith(
        'tenant-1',
        expect.any(Array),
      );
    });
    ...
    it('accepts optional metrics filter', async () => {
      const app = createTestApp();
      const res = await app.request('/api/v1/analytics/real-time?metrics=messages_sent,agent_sessions');
