...
healthcareRouter.get('/queue', requireAuth(), rateLimit('read'), async (c): Promise<Response> => {
...
healthcareRouter.get(
  ...
  async (c): Promise<Response> => {
    ...
    const counts = await db
      ...
      .where(
        and(
          eq(complianceRecords.tenantId, ctx.tenantId),
          eq(complianceRecords.regulation, 'hipaa'),
        ),
      )
...
healthcareRouter.get(
  ...
  async (c): Promise<Response> => {
    ...
    const rows = await db
      ...
      .from(agentSessions)
      .where(
        and(eq(agentSessions.tenantId, ctx.tenantId), like(agentSessions.agentRole, '%health%')),
      )
